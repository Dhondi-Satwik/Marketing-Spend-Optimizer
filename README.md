...
Designed and implemented a rules-first marketing budget decision system to allocate a fixed weekly budget across multiple channels under strict business constraints.
Built schema-enforced data pipelines for validation, weekly aggregation, and KPI computation (ROI, spend efficiency).
Implemented a deterministic rule engine with budget caps and conservation guarantees to ensure explainable and auditable decisions.
Integrated an ML advisory layer using interpretable regression to prevent premature budget cuts for channels with delayed revenue impact.
...
---
## System Philosophy
1. Rules own decisions 
2. ML is advisory only
3. Explainability > accuracy
4. Failure-safe > optimization
5. Schema contracts enforced at every stage
